...
rep = 1     #Number of COMPASScovariate replicates
sim.rep = 2
...
sim.pars <- 
  expand.grid(I = c(50),#, 100, 200),   #Number of subjects
              ...
              p = c(2),         #Number of covariates IN ADDITION TO AGE | HIV status
              # prob.hiv.pos = c(0.05, 0.1, 0.5),    #Proportion with binary indicator == 1
              beta.unif.min = c(-4),#c(0, -1, -2, -3, -4),         #Lower bound on beta1 distribution
              beta.unif.max = c(-3.8)#c(0, -0.8, -1.8, -2.8, -3.8)          #Upper bound on beta1 distribution
              
  ) %>%

  #Remove unwanted combinations
  filter((beta.unif.min <= beta.unif.max & dplyr::near(beta.unif.max - beta.unif.min, 0.2)) | (dplyr::near(beta.unif.min, 0) & dplyr::near(beta.unif.max, 0))
  ) %>%
  filter(!duplicated(.))
...
Compare sum of absolute error between models
```{r}
do.call('rbind',
...
Find subjects with highest absolute error by model and examine covariate values
```{r}
do.call('rbind',
  lapply(sim.out.list, FUN = function(sim) {
    #Omega - mean_gamma
    ...
    omega.diff.df %>%
      ...
      #Find the sum of absolute error for each subject across all cell subsets
      mutate(sum.abs.dev = rowSums(across(starts_with("cluster"),
                                     ~ abs(.x)))) %>%
      
      #Group by model
      group_by(model) %>%
      
      #Find the subject with the maximum sum of absolute error for each model
      slice_max(n = 1,
                sum.abs.dev
      ) %>%
      select(subject, model, sum.abs.dev) %>%
      
      #Join with the subject-level covariate dataset by subject
      inner_join(.,
                 sim$sim.fit$data$meta %>%
                   select(age2, V2, V3) %>%
                   tibble::rownames_to_column(var = "subject"),
                 by = "subject"
      ) %>%
      
      #Convert the covariate values to percentiles from the standard normal (they were created as such or scaled)
      mutate(across(starts_with(c("age", "V")),
                    ~ pnorm(q = .x))
      ) %>%
      
      #Add sim pars
      mutate(rep = sim$sim_rep,
             N.subj = sim$I,
             beta.unif.min = sim$beta.unif.min,
             beta.unif.max = sim$beta.unif.max
      ) %>%
      relocate(N.subj, .before = subject) %>%
      relocate(rep, .after = N.subj) %>%
      relocate(beta.unif.min, .after = rep) %>%
      relocate(beta.unif.max, .after = beta.unif.min)
...
Find subjects with max absolute difference for specific cell subsets for a specific simulation
```{r}
#Pick the first output for rep 1 with the most extreme beta values
sim = sim.out.list[[which(sim.pars$sim_rep == 1 & sim.pars$beta.unif.min == min(sim.pars$beta.unif.min))]]
#Omega - mean_gamma
omega.diff.covar <- abs(sim$omega - sim$sim.fit$fit$mean_gamma[,1:sim$K1])
omega.diff.compass <- abs(sim$omega - sim$compass.fit$fit$fit$mean_gamma[,1:sim$K1])
colnames(omega.diff.compass) <- NULL

##################################
#Find subjects with maximum values
covar.subj <-
which(omega.diff.covar == max(omega.diff.covar),
      arr.ind = TRUE
) %>%
as.data.frame() %>%
summarize(subject = unique(row)) %>%
mutate(model = "COMPASScov")

compass.subj <-
which(omega.diff.compass == max(omega.diff.compass),
      arr.ind = TRUE
) %>%
as.data.frame() %>%
summarize(subject = unique(row)) %>%
...
### Variable Selection for Betas

**Goal:** Examine the proportion of intervals for beta that contain 0 for which the true beta is 0

- If true beta is 0, does interval contain 0?
- If true beta is not 0, does interval not contain 0?

**Note:** Only examine beta1 (slopes) because no variable selection for the true beta0 was performed, so none are exactly 0.
...
do.call('rbind', 
  lapply(sim.out.list, FUN = function(sim) {
    ...
    data.frame(
      ...
      summarize(
                beta1.incl.0 = sum(true.mean == 0 & post.LCL.95 < 0 & post.UCL.95 > 0) / sum(true.mean == 0),
                beta1.excl.0 = sum(true.mean != 0 & ( post.UCL.95 < 0 | post.LCL.95 > 0)) / sum(true.mean != 0),
                true.0 = sum(true.mean == 0)
      ) %>%
      mutate(
             N.subj = sim$I,
             beta.unif.min = sim$beta.unif.min,
      ...
      relocate(N.subj, .before = beta1.incl.0) %>%
      relocate(beta.unif.min, .after = N.subj) %>%
      relocate(beta.unif.max, .after = beta.unif.min)
  })
) %>%
  
  group_by(N.subj, beta.unif.min, beta.unif.max) %>%
  summarize(N.rep = n(),
            Mean.beta1.incl.0 = mean(beta1.incl.0),
            Mean.beta1.excl.0 = mean(beta1.excl.0),
            Mean.true.0 = mean(true.0)
  )
```

## Prediction Accuracy

- Generate new data covariates
- Take mean, median, mode of beta posteriors. Or generate 
- Posterior sample of beta, to generate probs, then mean of probs
- Compare to true probs (omega)

**Note:** Aim to vary the number of covariates and examine how the prediction accuracy changes
...
dim(sim$X.test)
dim(sim$sim.fit$fit$beta[,,1])

apply(sim$sim.fit$fit$beta[,,1:2], MARGIN = c(1,3), FUN = function(beta) {
  sim$X.test
  # sim$X.test %*% beta
  # binomial()$linkinv(sim$X.test %*% t(beta))
})
```

```{r}
apply(sim$sim.fit$fit$beta, MARGIN = c(1,2), FUN = function(beta) {
  binomial()$linkinv(sim$X.test %*% t(beta))
})

lapply(1:2, FUN = function(i) {
  binomial()$linkinv(sim$X.test %*% t(sim$sim.fit$fit$beta[,,i]))
})

(apply(sim$sim.fit$fit$beta[,,1:2], MARGIN = c(1,2), FUN = function(beta) {
  # beta
  sim$X.test[1,]
  # binomial()$linkinv(sim$X.test %*% t(beta))
}))

apply(sim$sim.fit$fit$beta[1:2,1:2,1:2], MARGIN = c(1,2,3), FUN = function(beta) {
  # beta
  sim$X.test[1,]
  # sim$X.test %*% t(sim$sim.fit$fit$beta[,,1])
})

```

```{r}
get.probs <- function(mat) {
  return(binomial()$linkinv(sim$X.test %*% t(mat)))
}

apply(sim$sim.fit$fit$beta[,,1:2], MARGIN = c(3), FUN = get.probs)

apply(sim$sim.fit$fit$beta[1:2,1:2,1:2], MARGIN = c(1,2), FUN = function(x) {
  x
})
...
apply(my.mat, MARGIN = 1, FUN = function(x) {
  my.vec
  # x %*% my.vec
})
...
```{r}
#Betas -- 15 subsets, 4 covariates, 2 sets
my.betas <- 
  array(data = sample(c(0,1),
                      size = 15*4*2,
                      replace = TRUE
        ),
        dim = c(15,4,2)
...
#Covariates -- 3 samples 4 covariates
my.cov <- 
  matrix(data = sample(c(0,1),
                       size = 4*50,
                       replace = TRUE
         ),
         nrow = 50,
...
dim(apply(my.betas, MARGIN = c(1,2), FUN = function(x) {
  my.cov.t
}))

out <- apply(my.betas, MARGIN = c(1,2), FUN = function(x) {
  x
})
out
```
...
```{r}
apply(out, MARGIN = c(1,2), FUN = function(x) {
  mean(x)
})
```
