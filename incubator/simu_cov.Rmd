...
iter = 10000 #Number of COMPASScovariate iterations per replicate
...
sim.pars <- 
  expand.grid(I = c(50),#, 60, 100, 200),   #Number of subjects
              ...
              prob.hiv.pos = c(0.1, 0.5),    #Proportion with binary indicator == 1
              beta.unif.min = c(6, 3),         #Lower bound on beta1 distribution
...
do.call('rbind',
  ...
  #Calculate percentage correct, over-, and under-estimated gammas
  gamma.correct <- 
    bind_rows(
      true.gamma.diff.covar.df %>%
        select(-hiv) %>%
        summarize(
          Per.correct = sum(apply(., MARGIN = 2, FUN = function(x) {
                             sum(x == 0)
                             }
                       )
          ) / (dim(.)[1] * dim(.)[2]) * 100,
          Per.under = sum(apply(., MARGIN = 2, FUN = function(x) {
                             sum(x < 0)
                             }
                       )
          ) / (dim(.)[1] * dim(.)[2]) * 100,
          Per.over = sum(apply(., MARGIN = 2, FUN = function(x) {
                             sum(x > 0)
                             }
                       )
          ) / (dim(.)[1] * dim(.)[2]) * 100
    ...
    #Extract just percent correct gamma and display
    gamma.correct %>%
      select(-Per.under, -Per.over) %>%
      pivot_wider(names_from = Model,
                  values_from = Per.correct,
                  id_cols = NULL
      ) %>%
      ...
      relocate(COMPASScov, .after = Beta.unif.max) %>%
      relocate(COMPASS, .after = COMPASScov)
