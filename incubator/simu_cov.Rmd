...
sim.pars <- expand.grid(I = c(50),#, 60, 100, 200),   #Number of subjects
...
for (i in 1:I) {
  ...
  # p_s[i, c(which(gamma.m[i,] == 1), K)] <- rdirichlet(n = 1, alpha = alpha_s[i, c(which(gamma.m[i,] == 1), K)]) * sum(p_u[i, c(which(gamma.m[i,] == 1), K)])
  # p_s[i, c(which(gamma.m[i,] == 1), K)] <-
  #   rdirichlet(n = 1,
  #          alpha = c(alpha_s[i, which(gamma.m[i,] == 1)] * min(sum(gamma.m[i,] == 1) / (K/4), 1),
  #                    alpha_s[i, K]
  #                   )
...
Testing methods for adjusting alpha
...
plot.df <- 
  data.frame(
    ng.obs = seq.default(from = 1, to = 10, by = 0.01)
  ) %>%
  rowwise() %>%
  mutate(alpha = conc.adjust(ng = ng.obs, k = K, alpha = 200, p = 0.25),
         prob = rdirichlet(n = 1, alpha = c(alpha, 500))[1]
  )
```
...
plot.df %>%
  ...
  geom_point()+
  geom_hline(yintercept = mean(rdirichlet(n = 100, alpha = c(200, 500))[,1]))+
  scale_x_continuous(breaks = seq.default(from = 0, to = 10, by = 1))
...
diff <- (n_s / rowSums(n_s)) - (n_u / rowSums(n_u))

#Proportion of counts in each subset for stimulated minus proportion of counts in each subset for unstimulated
diff.test <- (n_s[1,] / sum(n_s[1,])) - (n_u[1,] / sum(n_u[1,]))

#Make a dataframe
diff.df <- data.frame(hiv = as.factor(covar$hiv),
                      diff
)
