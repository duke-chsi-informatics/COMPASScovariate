...
sim.pars <- 
  expand.grid(I = c(50),#, 60, 100, 200),   #Number of subjects
              ...
              beta.unif.min = c(6, 3.5),         #Lower bound on beta1 distribution
              beta.unif.max = c(8, 4)          #Upper bound on beta1 distribution
              
  ) %>%
  
  #Remove unwanted combinations
  filter(beta.unif.min <= beta.unif.max &
         beta.unif.max - beta.unif.min <= 2
  )
...
lapply(sim.out.list, FUN = function(fit) {
  ...
  do.call('rbind', .) %>%
  group_by(per.covar, beta.unif.min, beta.unif.max, metric) %>%
...
```{r eval=FALSE, include=FALSE}
sim.out.list[[3]]$beta.true.final



sim.out.list[[3]]$gamma.m[, 5:6] - sim.out.list[[3]]$sim.fit$fit$mean_gamma[, 5:6]

sim.out.list[[3]]$gamma.m[, 5:6] - sim.out.list[[3]]$compass.fit$fit$fit$mean_gamma[, 5:6]
```
...
lapply(sim.out.list, FUN = function(sim) {
  ...
  # gridExtra::grid.arrange(grobs = plot.out.list,
  #                         ncol = 4
  # )
  
  gr <- gridExtra::marrangeGrob(grobs = plot.out.list,
                                ncol = 4,
                                nrow = 4
  )
...
lapply(sim.out.list, FUN = function(sim) {
  ...
  gr <- gridExtra::marrangeGrob(grobs = hist.plot.list,
                                ncol = 2,
                                nrow = 8
  )
...
do.call('rbind',
  lapply(sim.out.list, FUN = function(sim) {
    ...
    #Create datasets
    omega.diff.covar.df <- data.frame(hiv = as.factor(sim$sim.fit$data$meta$hiv),
                                            omega.diff.covar
    )
    omega.diff.compass.df <- data.frame(hiv = as.factor(sim$sim.fit$data$meta$hiv),
                                              omega.diff.compass
    )
    ...
    omega.diff.df %>%
      ...
      mutate(SSE.total = rowSums(across(starts_with("cluster")))
      ) %>%
      ...
      bind_rows(summarise(.,
                      across(where(is.numeric), ~ (1 - (.x[model == "COMPASScovariate"] / .x[model == "COMPASS"])) * 100),
                      across(where(is.character), ~ "COMPASScovariate / COMPASS")
                )
      ) %>%
      
      relocate(SSE.total, .after = model) %>%
      ...
      relocate(beta.unif.min, .after = per.covar) %>%
  ...
  group_by(per.covar, beta.unif.min, beta.unif.max, model) %>%
  summarize(N.rep = n(),
            mean.SSE.total = mean(SSE.total)
  )
```
...
lapply(sim.out.list, FUN = function(sim) {
  ...
  # return(hist.plot.list)
  # gridExtra::grid.arrange(grobs = hist.plot.list,
  #                         ncol = 2
  # )
...
### Gamma from COMPASScovariate vs COMPASS
...
lapply(sim.out.list, FUN = function(sim) {
  ...
  true.gamma <- as.data.frame(sim$gamma.m)
  
  #Rename columns
  colnames(mean.gamma.diff.df) <- gsub("^X", "cluster", colnames(mean.gamma.diff.df))
  colnames(true.gamma) <- gsub("^V", "cluster", colnames(true.gamma))
  
  mean.gamma.diff.df %>%
    mutate(obs = row.names(.)) %>%
    pivot_longer(values_to = "Diff",
  ...
  #Histograms
  plot.list <-
    lapply(grep('cluster', colnames(mean.gamma.diff.df)[-ncol(mean.gamma.diff.df)], value = T), FUN = function(cluster) {
      ...
      temp <-
        mean.gamma.diff.df %>%
          ...
          rename(est = {{cluster}}) %>%
          mutate(true = as.factor(true.gamma[, colnames(true.gamma) == {{cluster}}])
          )
  ...
  gr <- gridExtra::marrangeGrob(grobs = plot.list,
                          ncol = 2,
                          nrow = 8
  )
...
do.call('rbind', 
  lapply(sim.out.list, FUN = function(sim) {
    ...
    do.call('rbind',
      ...
      apply(
        ...
        apply(sim$compass.fit$fit$fit$gamma, MARGIN = c(1,2), var) - 
          apply(sim$sim.fit$fit$gamma, MARGIN = c(1,2), var),
        MARGIN = 2,
        FUN = function(x) {
          
          #Make a dataframe of the sum of the diff in variance for all subsets
          dat = data.frame(
            diff.precision = sum(x)
          )
  ...
  group_by(N.subjects, Prob.covar, Beta.unif.min, Beta.unif.max) %>%
  summarize(N.rep = n(),
            Mean.total.diff.var = mean(total.diff.var)
  )
```
...
do.call('rbind', 
  ...
  group_by(per.covar, beta.unif.min, beta.unif.max, param) %>%
  summarize(N.rep = n(),
