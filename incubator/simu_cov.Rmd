...
```{r}
set.seed(111)
#Get number of threads from SLURM scheduler -- set in 'run_simu_cov.sh'
threads = Sys.getenv("SLURM_CPUS_PER_TASK")
...
```{r}
sim.pars <- expand.grid(I = c(30, 60, 100, 200),
                        K = c(4),
                        p = c(5)
            )
```
...
lapply(sim.out.list, FUN = function(sim) {
  ...
  mod.fit.list <- data.frame(
                    ...
                    Percent.beta.coverage = percent.beta.coverage,
