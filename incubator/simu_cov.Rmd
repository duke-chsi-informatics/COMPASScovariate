...
```{r}
summary(sim.out.list[[1]]$sim.fit$fit$A_gamma)

summary(sim.out.list[[1]]$compass.fit$fit$fit$A_gamma)
```
...
lapply(sim.out.list, FUN = function(sim) {
  ...
  qplot(x = diff.df$`diff[, 1]`,
        y = as.factor(sim$sim.fit$data$meta$hiv)
...
lapply(sim.out.list, FUN = function(sim) {
  ...
  #Plot as a function of HIV status
  qplot(x = mean.gamma.diff.df$cluster1,
...
```{r}
dim(sim.out.list[[1]]$sim.fit$fit$mean_gamma)

#Posterior draws for gamma[1,1] from COMPASScovariate


#Posterior draws for gamma[1,1] from COMPASS
lapply(1:K, FUN = function(k) {
  print(
    paste(
      c("Var for COMPASScovariate:",
        var(sim.out.list[[1]]$sim.fit$fit$gamma[2,k,]),
        "Var for COMPASS:",
        var(sim.out.list[[1]]$compass.fit$fit$fit$gamma[2,k,])
      )
    )
  )
})
...
```{r}
(sim.out.list[[1]]$sim.fit$data$n_s / rowSums(sim.out.list[[1]]$sim.fit$data$n_s)) - (sim.out.list[[1]]$sim.fit$data$n_u / rowSums(sim.out.list[[1]]$sim.fit$data$n_u))
```
