...
**Approach**
  ...
  2. Vector of true intercepts for each k-1 subset ~ Normal(int.mu, int.sd)
    1. int.mu is 0 to keep probability that gamma = 1 ~ 50%, which makes estimating betas easier.
...
```{r}
beta.diff <- abs(beta.true.final - sim.fit.beta.median)
...
Extract and plot posterior samples for flagged coefs (if any)
```{r}
if (length(beta.poor.coord.df) > 0) {
