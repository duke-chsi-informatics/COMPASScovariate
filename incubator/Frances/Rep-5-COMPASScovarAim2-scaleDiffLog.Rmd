...
# 17/20 patients for Aim 1
aim1DF <-antibodyDF %>%
  inner_join(pp65_demoInfo_Aim1) %>%
  inner_join(pp65)

#41/44 patients for Aim 2
aim2DF <- antibodyDF %>%
...
flowAim2 <- pp65 %>%
  ...
  group_by(TheID) %>%
  filter(Visit == max(Visit)) %>% ungroup()
...
# returns vector of stim, unstim, and cat matrices
stim_unstim_cats <- function(flowAim2) {
 #--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
  # metadata 
  #--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
  metadat<- flowAim2 %>% select(TheID, Stimulation, ID)
  
  
  #--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
  # unique ID 
  #--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
  uniqueID <- flowAim2$ID 
...
To further define our antibody covariate matrix, we get rid of mostly missing covariates and some correlated covariates then add age as a covariate, saving as the dataframe "antibodyDF_diffs".

```{r -twoThree-antibodies, options}
# the antibody variables of interest (exclude mostly NAs)
antibodyDF_diffs_all<- antibodyDF_logScale_diffs %>%
...
```{r cov-corr, options}
cor(antibodyDF_diffs_all %>% select(-c(TheID,Visit))) %>%
  as.data.frame() %>%
  rownames_to_column() %>% 
...
antibodyDF_diffs <- antibodyDF_diffs_all %>%
  select(-c(`gH/gL (MFI)`,
            `gH/gL/gO (MFI)`,
...
```{r collapse-gates, options}

#------------------------------------------------
# Parent flow containing Aim 2 CD4 and CD8 gates
#------------------------------------------------
flowAim2 <- flowAim(aim2DF)

#------------------------------------------------
# Collapse CD4 and CD8 gates
# Use directly in COMPASS for PFS or cut further
#------------------------------------------------
...
#-------------------------------------------
cutGates <- function(flowAim2) {
  # gate names to be excluded
  excludedCellCats <- flowAim2 %>%
    inner_join(antibodyDF_diffs %>% select(TheID)) %>%
  ...
  excludedCellCounts <- data.frame(exCount=flowAim2 %>% 
  inner_join(antibodyDF_diffs %>% select(TheID)) %>%
  ...
  select(-all_of(excludedCellCats)) %>%
# add back excluded cells
  cbind(excludedCellCounts) %>%
  mutate(across(matches("CD4\\+|-CD107a-IFNg-IL-2-TNFa-"), ~ .x + exCount)) %>%
  select(-exCount) %>%
  cbind(flowAim2 %>% 
  inner_join(antibodyDF_diffs %>% select(TheID)) %>% select(ID, TheID, Visit, Stimulation)) %>%
...
flowAim2_cut <- cutGates(flowAim2)
# flow data with cut CD4 gates 
flowAim2_cut_CD4 <- cutGates(CD4gates_all)
# flow data with cut CD8 gates 
flowAim2_cut_CD8 <- cutGates(CD8gates_all)

  
```

```{r corr-abd-flow, options}
#---------------------------------------------------
# FUNCTION for combining cut gates with antibody data
#---------------------------------------------------
flowAim2_diffsA <- function(flowAim2_cut) {
  return(flowAim2_cut %>%
  inner_join(antibodyDF_diffs %>% select(TheID)))
}
...
#--------------------------------------------------------------------------------
corrPlot <- function(flowAim2_cut) {
...
flowAim2_diffsAnti_freqs <- flowAim2_diffsAnti %>%
...
flowAim2_diffsAnti_diffs <- flowAim2_diffsAnti_freqs %>%
  group_by(TheID) %>%
...
corPlot <- cor(flowAim2_diffsAnti_diffs %>%
  inner_join(antibodyDF_diffs) %>% ungroup() %>% select(-c(Visit,TheID))) %>%
  as.data.frame() %>%
  rownames_to_column() %>%
...
#-------------------------------------------------
COMPASS_OG <- function(flowAim2_cut, postfix) {
  flowAim2_diffsAnti <- flowAim2_diffsA(flowAim2_cut)
  starttime <- Sys.time()
  pp65COMPASSAim2 <- COMPASS_clean("pp65", flowAim2_diffsAnti, iters = 200000,1)
  categs <- pp65COMPASSAim2$fit$categories
  endtime <- Sys.time()
  
  saveRDS(list(pp65COMPASSAim2_diffs = pp65COMPASSAim2, runTime = endtime-starttime),
          paste0("~/CHSI/compass-covariate/data/pp65Aim2diffsCOMPASS200000_seed100_",postfix,".rds"))
...
The X matrix we feed into COMPASS covariate is the matrix of `r ncol(antibodyDF_logScale_diffs %>% select(-c(TheID, Visit)))` available antibody markers for our 20 patients. A glimpse of the input for X is below:
...
X <- antibodyDF_diffs %>% 
  ...
  # select(`gH/gL (MFI)`,
  #        `Pentameric Complex (MFI)`,
  #        `CMV-specific IgG3 Concentration (ug/ml)`, ID50, `Area Under Curve.x`) %>% as.matrix()

head(round(X,2)) %>% datatable()

#-------------------------------------------------------------------------
# FUNCTION for COMPASS cpvariate
# takes in flow data frame, and suffix for naming rds file
#-------------------------------------------------------------------------
COMPASS_covar <- function(flowAim2_cut, postfix) {
  flowAim2_diffsAnti <- flowAim2_diffsA(flowAim2_cut)
  
  stim_unstim_cat_Aim2_diffs <- stim_unstim_cats(flowAim2_diffsAnti)
  
  # magic code to make it work...
  n_s <- stim_unstim_cat_Aim2_diffs$stim
  n_u <- stim_unstim_cat_Aim2_diffs$unstim
  categories <- stim_unstim_cat_Aim2_diffs$cats
  I <- nrow(n_s)
  
  # n_s (I x K), n_u (I x K), iters, replications, X (I x J)
  starttime = Sys.time()
  COMPASScovarAim2_diffs <- .COMPASS.covariate(n_s=n_s, n_u=n_u,
...
saveRDS(list(pp65COMPASSAim2_diffs = COMPASScovarAim2_diffs, runTime = endtime-starttime),
          paste0("~/CHSI/compass-covariate/data/pp65Aim2diffsCOMPASScovar200000_seed100_",postfix,".rds"))

  
# saveRDS(list(pp65COMPASSAim2_diffs = COMPASScovarAim2_diffs, runTime = endtime-starttime),
#           paste0("//duhs-vclin-nc1/dusom_biostats_fs/Data/biostatscore/CRU/Immunology Center/Palmer/Pro000102592/COMPASS-covar/data/pp65Aim2diffsCOMPASS200000_seed100_",postfix,".rds"))
rm(COMPASScovarAim2_diffs)
...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
betaTable <- function(COMPASScovarAim2_diffs, flowAim2_cut) {
  
  flowAim2_diffsAnti <- flowAim2_diffsA(flowAim2_cut)

nSamp <- nrow(COMPASScovarAim2_diffs$alpha_s)
betaSamps <- COMPASScovarAim2_diffs$beta[,,(nSamp/4):nSamp]
K1 = nrow(betaSamps[,,1])
P1 = ncol(betaSamps[,,1])
...
round(apply(array(unlist(betaSamps), c(K1, P1, nSamp-(nSamp/4)+1)), c(1,2), function(x) quantile(x, 0.975)),2), ")"), nrow = nrow(betaSamps[,,1])) %>%
  as.data.frame() %>%
  mutate(Gate = flowAim2_diffsAnti %>% 
    select(contains("Count")) %>% 
    rename_with( ~ str_remove_all(.x, "Count") %>%
                      str_remove_all("\\|"), contains("Count")) %>% colnames() %>% .[1:K1]) 
  

colnames(CIDF) <- c("Intercept", colnames(antibodyDF_diffs %>% select(-c(Visit, TheID))), "Gate")
...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
beta_heatmap <- function(COMPASScovarAim2_diffs, flowAim2_cut, percburn=0.5) {
  flowAim2_diffsAnti <- flowAim2_diffsA(flowAim2_cut)

nSamp <- nrow(COMPASScovarAim2_diffs$alpha_s)
betaSamps <- COMPASScovarAim2_diffs$beta[,,(round(nSamp*percburn)+1):nSamp]
K1 = nrow(betaSamps[,,1])
P1 = ncol(betaSamps[,,1])
...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
compareGammaBoxplot <- function(pp65COMPASSAim2, COMPASScovarAim2_diffs, percBurn=0.5) {

OGmeanGam <- mean_gam(pp65COMPASSAim2$fit$gamma, percBurn) %>%
  `colnames<-`(colnames(pp65COMPASSAim2$fit$mean_gamma))
CovarmeanGam <- mean_gam(COMPASScovarAim2_diffs$gamma, percBurn) %>%
  `colnames<-`(colnames(COMPASScovarAim2_diffs$mean_gamma))
...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
getBetaLong <- function(compasscovarBeta, percburn, thin=33) {
  #--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
...
betaTrace <- function(compasscovarBeta, gateNames, percburn=0.5, thin=33) {
  
  betaSamps <- getBetaLong(compasscovarBeta, percburn, thin) %>%
...
#COMPASS_OG(flowAim2_cut_CD4, "more3forhalf_CD4")
...
pp65COMPASSAim2_CD4<- readRDS("~/CHSI/compass-covariate/data/archive/pp65Aim2preTCOMPASS200000_seed100_more3forhalf_CD4.rds")$pp65COMPASSAim2_preT
```
...
flowAim2_diffsAnti <- flowAim2_diffsA(flowAim2_cut_CD4)
...
#COMPASS_covar(flowAim2_cut_CD4,"more3forhalf_CD4_covar")
...
COMPASScovarAim2_diffs_CD4<- readRDS("~/CHSI/compass-covariate/data/archive/pp65Aim2diffsCOMPASScovar200000_seed100_more3forhalf_CD4_covar.rds")$pp65COMPASSAim2_diffs


compareGammaBoxplot(pp65COMPASSAim2_CD4, COMPASScovarAim2_diffs_CD4)
```
...
changingPats_CD4 <- compareGammaDF(pp65COMPASSAim2_CD4, COMPASScovarAim2_diffs_CD4) %>%
...
cbind(x=pp65COMPASSAim2_CD4$fit$A_gamma, y=COMPASScovarAim2_diffs_CD4$A_gamma) %>%
  as.data.frame() %>%
  ggplot(aes(x=x,y=y)) +
  geom_point() +
  geom_abline() +
...
betaTrace(COMPASScovarAim2_diffs_CD4$beta, 
...
corrPlot(flowAim2_cut_CD4)
beta_heatmap(COMPASScovarAim2_diffs_CD4, flowAim2_cut_CD4)
```
...
#COMPASS_OG(flowAim2_cut_CD8, "more3forhalf_CD8")
...
pp65COMPASSAim2_CD8 <- readRDS("~/CHSI/compass-covariate/data/archive/pp65Aim2preTCOMPASS200000_seed100_more3forhalf_CD8.rds")$pp65COMPASSAim2_preT
```
...
flowAim2_diffsAnti <- flowAim2_diffsA(flowAim2_cut_CD8)
...
set.seed(100)
#COMPASS_covar(flowAim2_cut_CD8,"more3forhalf_CD8_covar")
...
COMPASScovarAim2_diffs_CD8<- readRDS("~/CHSI/compass-covariate/data/archive/pp65Aim2diffsCOMPASScovar200000_seed100_more3forhalf_CD8_covar.rds")$pp65COMPASSAim2_diffs


compareGammaBoxplot(pp65COMPASSAim2_CD8, COMPASScovarAim2_diffs_CD8)
```

The acceptance rate for gamma for COMPASS is plotted on the below x-axis and the acceptance rate for gamma for COMPASS-covariate is plotted on the below y-axis.

```{r gamma-a-CD8, options}
cbind(x=pp65COMPASSAim2_CD8$fit$A_gamma, y=COMPASScovarAim2_diffs_CD8$A_gamma) %>%
  as.data.frame() %>%
  ggplot(aes(x=x,y=y)) +
  geom_point() +
  geom_abline() +
...
betaTrace(COMPASScovarAim2_diffs_CD8$beta, 
...
corrPlot(flowAim2_cut_CD8)
beta_heatmap(COMPASScovarAim2_diffs_CD8, flowAim2_cut_CD8)
```
...
PFSCD4covar <- PolyfunctionalityScoreCD4or8(list(fit=COMPASScovarAim2_diffs_CD4), "^CD4") %>% as.data.frame() 

PFSCD8covar <- PolyfunctionalityScoreCD4or8(list(fit=COMPASScovarAim2_diffs_CD8), "^!CD4") %>% as.data.frame()
