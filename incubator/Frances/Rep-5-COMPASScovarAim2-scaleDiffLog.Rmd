...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
# In this code chunk, we find patients who have both abd and flow
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
...
We consider including the patients below, who don't stop prophy but who have flow at 2/3 months and antibody measurements at pre-transplant and 2/3 months. There are currently none; there are 19 patients with the flow data but no antibody data. TODO: ask Melissa if these are available.

```{r toInclude-woProphyStop, fig.height=20}
pp65 %>% inner_join(NewBoolInfo) %>% 
  inner_join(Prophlax,by="SUBJID") %>% 
  filter(is.na(PROPDUR)) %>% filter(Visit %in% c(1,2)) %>%
# join with antibody
  inner_join(antibodyDF)
```
...
flow_freqDiffs <- function(flowDF_cut) {
  ...
  #TODO: check if this is the right order
    summarise(across(contains("Freq"), ~ .x-lag(.x))) %>%
    drop_na()
}
...
# is this the best way to deal with zero-valued data?
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
# Antibody (pre-transplant) data for Aim 2: antibodyDF_log_PreT
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
...
# Antibodies at months 2 or 3 for Aim 2 
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
antibodyDF_twoThree <- antibodyDF_PreT %>% select(TheID) %>%
  left_join(antibodyDF %>%
              filter(Visit %in% c(1,2))) %>%
  group_by(TheID) %>%
  filter(Visit == max(Visit)) %>%
  ungroup() 
...
# Scaled log(post-tx antibodies) minus log(pre-tx antibodies)
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
antibodyDF_diffs <- antibodyDF_twoThree %>% select(TheID, Visit) %>%
  cbind(antibodyDF_twoThree %>% select(-c(TheID, Visit)) - 
  antibodyDF_PreT %>% select(-c(TheID, Visit)))

antibodyDF_diffs %>%
  select(TheID, `gB Ectodomain (MFI)`,
         `Phagocytosis Score`,
         `OD450 nm`,
         `ID50 with complement`,
         `Area Under Curve.x`, 
         Avidity,
         `CMV-specific IgG3 Concentration (ug/ml)`) %>%
  pivot_longer(-TheID, names_to="Var") %>%
  ggplot(aes(x=value)) + geom_histogram() +
  facet_wrap(.~Var, scales = "free") +ggtitle("Raw Differences in Antibody Covariates")
```
...
To further define our antibody covariate matrix, we get rid of mostly missing covariates and some correlated covariates then add age as a covariate, saving as the dataframe "antibodyDF\_".
...
cor(antibodyDF_noNAs %>% select(-c(TheID,Visit))) %>%
  ...
  geom_tile() + theme(axis.text.x = element_text(angle = 30, hjust = 1, vjust = 1)) +
  scale_fill_gradient2(low = "purple", mid= "white",high = "red") +
  ggtitle("Correlation between Transformed Antibody Measures")
```
...
flowFreqDiffs(flowAim2_cut_CD4) %>%
  ...
  ggplot(aes(x= values)) + 
  geom_histogram() + geom_vline(data=changingFlow, aes(xintercept=values, color=TheID)) + 
...
changingAbd <- antibodyDF_finalCD4 %>%
  select(TheID, Visit,AGE,`gB Ectodomain (MFI)`,
         `Phagocytosis Score`,
         `OD450 nm`,
         `ID50 with complement`,
         `Area Under Curve.x`, 
...
antibodyDF_finalCD4 %>%
  ...
  left_join(changingPats_CD4 %>% select(TheID, Flag)) %>%
  ggplot(aes(x= values)) + 
  geom_histogram() + geom_vline(data=changingAbd, aes(xintercept=values, color=TheID)) + 
  facet_grid(rows=vars(Vars), scales = "free") +
...
```{r antibody-diffs-flaggedpats, fig.height=15}
#antibodies of patients who change
changingAbd_raw_CD4 <- antibodyDF_diffs%>%
  select(-c(`CMV-specific IgG Concentration (ug/ml)`,`CMV-specific IgG4 Concentration (ug/ml)`)) %>%
  pivot_longer(cols = -c(TheID, Visit), names_to = "Vars", values_to = "values") %>%
  left_join(changingPats_CD4 %>% select(TheID, CovarlessThreshold, Flag)) %>%
  rename(`Lower Covariate Mean Gamma` = CovarlessThreshold) %>%
  filter(Flag)
...
antibodyDF_diffs %>%
  ...
  left_join(changingPats_CD4 %>% select(TheID, Flag)) %>%
  ggplot(aes(x= values)) + 
  geom_histogram() + geom_vline(data=changingAbd_raw_CD4, aes(xintercept=values, color=TheID)) + 
...
corrPlot(flowAim2_cut_CD4,antibodyDF_finalCD4)
...
betaTable(COMPASScovarAim2_CD4, flowAim2_cut_CD4, X_CD4)
```

As a visual check, we make scatterplots of patient age and gB ectodomain vs. difference in flow gate proportion.

```{r scatterplot-sig-cd4, options}
flow_freqDiffs(flowAim2_cut_CD4) %>%
  pivot_longer(-TheID, names_to = "Gates", values_to = "FreqDiffs") %>%
  left_join(antibodyDF_finalCD4 %>% select(TheID, AGE, `gB Ectodomain (MFI)`)) %>%
  left_join(pp65_demoInfo_Aim2 %>% select(TheID, event)) %>%
  mutate(Status = ifelse(event==1, "Infected", "Not Infected")) %>%
  ggplot(aes(x=AGE, y = FreqDiffs, color=Status)) + geom_point() +
    facet_wrap(.~Gates, scales = "free")
...
flow_freqDiffs(flowAim2_cut_CD4) %>%
  ...
  left_join(pp65_demoInfo_Aim2 %>% select(TheID, event)) %>%
  mutate(Status = ifelse(event==1, "Infected", "Not Infected")) %>%
  ggplot(aes(x=`gB Ectodomain (MFI)`, y = FreqDiffs, color=Status)) + geom_point() +
    facet_wrap(.~Gates, scales = "free")
```

We can also plot boxplots of the differences in flow proportions (pp65-noStim) for each gate.

```{r boxplot-flow-diffs, options}
flow_freqDiffs(flowAim2_cut_CD4) %>%
...
There are two patients who have outliers in difference between pp65 and noStim. We arrange proportions in pp65/noStim in descending order to see where these outliers stem from; in this dataset, the proportion difference outliers stem from pp65. 

```{r outlier-loop, options}

#arrange pp65 by descending proportion
pp65_freq <- flowAim2_cut_CD4 %>% select(TheID, Visit, contains("Count")) %>%
  filter(Visit %in% c(1,2)) %>% 
  inner_join(pp65_demoInfo_Aim2) %>% 
  select(TheID, Visit, matches("^CD4.*(\\+|-).*(\\+|-).*(\\+|-).*(\\+|-).*(\\+|-).*")) %>%
  mutate(sumCounts = rowSums(across(contains("Count")))) %>%
  # proportion across all gates (because COMPASS includes the CD4 vs.CD8 gates)
    mutate(across(contains("Count"), ~ .x/sumCounts)) %>%
    select(-c(sumCounts)) %>%
...
#arrange noStim by descending proportion
noStim_freq <- noStim %>% select(TheID, Visit, contains("Count")) %>%
  filter(Visit %in% c(1,2)) %>%
  inner_join(pp65_demoInfo_Aim2) %>% 
  select(TheID, Visit, matches("^CD4.*(\\+|-).*(\\+|-).*(\\+|-).*(\\+|-).*(\\+|-).*")) %>%
  mutate(sumCounts = rowSums(across(contains("Count")))) %>%
...
pp65_freq %>%
  pivot_longer(-c(TheID,Visit), names_to = "Gate", values_to = "Freq") %>%
  select(-Visit) %>%
  group_by(Gate) %>%
# take top 2 frequencies per gate
  arrange(desc(Freq)) %>%
  slice_head(n=3) #%>%
#   ungroup() %>%
#   group_by(TheID) %>%
# # count how many times each patient is in top two
#   summarise(mean_Freq=mean(Freq),
#             median_Freq=median(Freq),
#             n=n()) %>%
#   arrange(desc(n))

# "CTOT22-39-195", "CTOT22-39-189"
noStim_freq %>%
  pivot_longer(-c(TheID,Visit), names_to = "Gate", values_to = "Freq") %>%
  select(-Visit) %>%
  group_by(Gate) %>%
# take bottom 2 frequencies per gate
  arrange(desc(Freq)) %>%
  slice_tail(n=2) %>%
  ungroup() %>%
# count how many times each patient is in top two
...
pp65 %>% filter(TheID %in% c("CTOT22-39-195","CTOT22-39-189"), 
                Visit %in% c(1,2)) %>% select(-contains("Freq"),-contains("Count")) %>%
# join with flow visit data
  left_join(WhenIsFlow) %>%
  select(-name) %>%
  rename(FlowDate=value) %>% 
  write_csv("~/CHSI/PackageDev/COMPASScovariate/incubator/Frances/Data/Misc/outliers.csv")
...
```{r nostim-outliers, fig.height=20}
median_noStim_prop <- noStim %>%
  select(TheID, Visit, contains("Count")) %>%
  filter(Visit %in% c(1,2)) %>% 
  inner_join(pp65_demoInfo_Aim2) %>% 
  select(TheID, Visit, matches(".*(\\+|-).*(\\+|-).*(\\+|-).*(\\+|-).*(\\+|-).*")) %>%
  mutate(sumCounts = rowSums(across(contains("Count")))) %>%
  # proportion across all gates (because COMPASS includes the CD4 vs.CD8 gates)
    mutate(across(contains("Count"), ~ .x/sumCounts)) %>%
    select(-c(sumCounts)) %>%
    rename_with(~ gsub("Count", "Freq", .x)) %>%
  pivot_longer(-c(TheID,Visit), names_to = "Gates", values_to = "Freqs") %>%
  group_by(Gates) %>% summarise(med_freq=median(Freqs))

noStim %>% 
  select(TheID, Visit, contains("Count")) %>%
  filter(Visit %in% c(1,2)) %>%
  filter(TheID %in% c("CTOT22-39-195", "CTOT22-39-189")) %>% inner_join(pp65_demoInfo_Aim2) %>% 
  select(TheID, Visit, matches(".*(\\+|-).*(\\+|-).*(\\+|-).*(\\+|-).*(\\+|-).*")) %>%
  mutate(sumCounts = rowSums(across(contains("Count")))) %>%
  # proportion across all gates (because COMPASS includes the CD4 vs.CD8 gates)
    mutate(across(contains("Count"), ~ .x/sumCounts)) %>%
    ...
    rename_with(~ gsub("Count", "Freq", .x)) %>%
  pivot_longer(-c(TheID,Visit), names_to = "Gates", values_to = "Freqs") %>%
# join with median pp65
  left_join(median_noStim_prop) %>% 
  ggplot(aes(x=Freqs)) + geom_histogram() + 
  geom_vline(aes(xintercept=med_freq)) + 
  facet_wrap(.~Gates, scales="free",ncol=3)
```
...
```{r mean-gamma-changes-CD8, fig.height=12}
#patients whose mean gamma changes past the 0.5 threshold for at least one gate, or whose mean gamma changes by at least 0.2
changingPats_CD8 <- compareGammaDF(pp65COMPASSAim2_CD8, COMPASScovarAim2_CD8) %>%
  pivot_wider(names_from = COMPASStype, 
              values_from = MeanGamma) %>%
  mutate(OGlessThreshold = Original < 0.5,
         CovarlessThreshold = Covar < 0.5,
...
#flow frequency differences of patients who change
changingFlow <-flowFreqDiffs(flowAim2_cut_CD8) %>%
  ungroup() %>%
  pivot_longer(cols = -c(TheID), names_to = "Vars", values_to = "values") %>%
  left_join(changingPats_CD8 %>% 
              select(TheID, CovarlessThreshold, Flag)) %>%
  rename(`Lower Covariate Mean Gamma` = CovarlessThreshold) %>%
  filter(Flag)

flowFreqDiffs(flowAim2_cut_CD8) %>%
  ungroup() %>%
  pivot_longer(cols = -c(TheID), names_to = "Vars", values_to = "values") %>%
  drop_na() %>%
  mutate(lessthan0Diff = values <0) %>%
  left_join(changingPats_CD8 %>% select(TheID, Flag)) %>%
  ggplot(aes(x= values)) + 
  geom_histogram() + geom_vline(data=changingFlow, aes(xintercept=values, color=TheID)) + 
  facet_grid(rows=vars(Vars),scales="free") +
  ggtitle("Changed Patients: Differences in pp65-noStim Flow") +
...
#antibodies of patients who change
changingAbd <- antibodyDF_finalCD8 %>%
  pivot_longer(cols = -c(TheID, Visit), names_to = "Vars", values_to = "values") %>%
  left_join(changingPats_CD8 %>% select(TheID, CovarlessThreshold, Flag)) %>%
  rename(`Lower Covariate Mean Gamma` = CovarlessThreshold) %>%
  filter(Flag)
...
corrPlot(flowAim2_cut_CD8,antibodyDF_finalCD8)
...
```{r boxplot-flow-diffs-CD8, options}
flow_freqDiffs(flowAim2_cut_CD8) %>%
  pivot_longer(cols=-TheID, names_to="Gate", values_to = "Difference") %>%
  ggplot(aes(x=Gate, y=Difference)) + geom_boxplot() +
  coord_flip()
...
# joining with demographic information
  inner_join(pp65_demoInfo_Aim2) %>%
# bind with additional antibody DF covariates
  left_join(antibodyDF_noNAs %>% select(TheID, `Area Under Curve.y`,                             `CMV-specific IgG2 Concentration (ug/ml)`,
                           ID50))
```
...
logModCD4 <- glm(event ~ PFSCD4 + `Area Under Curve.y` + `CMV-specific IgG2 Concentration (ug/ml)` + ID50,
                 family = "binomial", df)



logModCD8 <- glm(event ~ PFSCD8,
                 family = "binomial", df)




logModCD4covar <- glm(event ~ PFSCD4covar + `Area Under Curve.y` + `CMV-specific IgG2 Concentration (ug/ml)` + ID50,
                 family = "binomial", df)



logModCD8covar <- glm(event ~ PFSCD8covar,
                 family = "binomial", df)

summary(logModCD4)$coef %>% 
  as.data.frame() %>% select(-`z value`) %>% 
  bind_rows(summary(logModCD8)$coef %>% 
              as.data.frame() %>% select(-`z value`)) %>%
  bind_rows(summary(logModCD4covar)$coef %>% 
              as.data.frame() %>% select(-`z value`)) %>%
  bind_rows(summary(logModCD8covar)$coef %>% 
              as.data.frame() %>% select(-`z value`)) %>%
  rownames_to_column() %>%
  # `rownames<-`(c( "CD4PFS", 
  #                "CD8PFS","CD4PFS-Covar",
...
# %>%
  ...
  kable_classic() %>%
  pack_rows("CD4", start_row=1, end_row=5) %>%
  pack_rows("CD8", start_row=6, end_row=7) %>%
  pack_rows("CD4-Covar", start_row=8, end_row=12) %>%
  pack_rows("CD8-Covar", start_row=13, end_row=14)

}

logistic_results(pp65_Aim2_PFS)
```

### PFS Scores (including 3 "Independent" covariates)

```{r pfs-including-indep, options}
COMPASScovarAim2_CD4_OG <- readRDS("~/CHSI/compass-covariate/data/pp65Aim2COMPASScovar200000_seed100_diffLogs_noIndepFiltering_CD4.rds")$COMPASScovarobj

COMPASScovarAim2_CD8_OG <- readRDS("~/CHSI/compass-covariate/data/archive/pp65Aim2diffsCOMPASScovar200000_seed100_more3forhalf_CD8_covar.rds")$pp65COMPASSAim2_diffs


PFSCD4covar <- PolyfunctionalityScoreCD4or8(list(fit=COMPASScovarAim2_CD4_OG), "^CD4") %>% as.data.frame() 

PFSCD8covar <- PolyfunctionalityScoreCD4or8(list(fit=COMPASScovarAim2_CD8_OG), "^!CD4") %>% as.data.frame()
...
```{r log-models-no-indep, options}

df <- pp65_Aim2_PFS
logModCD4 <- glm(event ~ PFSCD4 ,
                 family = "binomial", df)
...
logModCD4covar <- glm(event ~ PFSCD4covar,
                 family = "binomial", df)
...
# %>%
  kable(col.names = c("Coefficient",
                      ...
                      "SE of Coefficient", "P-value"), digits = 3) %>%
  kable_classic() %>%
  pack_rows("CD4", start_row=1, end_row=2) %>%
  pack_rows("CD8", start_row=3, end_row=4) %>%
  pack_rows("CD4-Covar", start_row=5, end_row=6) %>%
  pack_rows("CD8-Covar", start_row=7, end_row=8)
...
FSCD4covar <- FunctionalityScore(list(fit=COMPASScovarAim2_CD4)) %>% as.data.frame() 
FSCD8covar <- FunctionalityScore(list(fit=COMPASScovarAim2_CD8)) %>% as.data.frame() 
...
```{r agg-FS, eval=FALSE}
df <- pp65_Aim2_FS
logModCD4 <- glm(event ~ FSCD4 ,
                 family = "binomial", df)



logModCD8 <- glm(event ~ FSCD8,
                 family = "binomial", df)




logModCD4covar <- glm(event ~ FSCD4covar,
                 family = "binomial", df)



logModCD8covar <- glm(event ~ FSCD8covar,
                 family = "binomial", df)
...
# Univariate Mean Gamma Logistic {.tabset .tabset-dropdown}
...
```{r univar-parent-loop, options}
# out = NULL
# 
# gateCOMPASScombos <- compareGammaDF(pp65COMPASSAim2_CD4, COMPASScovarAim2_CD4) %>%
#   select(Gate, COMPASStype) %>% distinct()
# 
# for (i in 1:nrow(gateCOMPASScombos)) {
#   
#   gateName<- gateCOMPASScombos[i,]$Gate
#   compassType <- gateCOMPASScombos[i,]$COMPASStype
# 
#   out = c(out, 
#           knitr::knit_expand('~/CHSI/PackageDev/COMPASScovariate/incubator/Frances/scaleDiff-univarLog.Rmd'))
#}
...
```{r mean-gam-logistic, options}
# gateName = "+--+"
# compassType = "Original"
# 
# singleGateGammaDF <- compareGammaDF(pp65COMPASSAim2_CD4, COMPASScovarAim2_CD4) %>%
#   mutate(event = ifelse(event=="Uninfected", 0, 1)) %>%
#   filter(Gate==gateName, COMPASStype==compassType) 
# 
# glm(event ~ MeanGamma, family = "binomial", data = singleGateGammaDF) %>%
#   broom::tidy() %>%
