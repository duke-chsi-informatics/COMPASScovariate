...
source("~/CHSI/PackageDev/COMPASScovariate/incubator/Frances/plot-helperscripts.R")
...
As a covariate(s) we can consider using the surface markers.
...
There are 23 CD4+ gates and 12 CD8+ gates included in the COMPASS analysis; we focus on the CD4+ gates under the NCAP condition.
...
In this section, we will use folds to divide the covariate matrices into train-test. The scaleCenterTest and PCATest functions will be used to transform the test covariates according to the train covariates.
...
#--------------------------------------------------------
# Test surface markers (logged and unlogged)
#--------------------------------------------------------
...
And the logged antibody distributions:
...
# uncomment below to run full COMPASS-covar
# cl <- makeCluster(3) # create a cluster with 3 workers
...
The run for full COMPASS-orignal:
...
Because we have three covariate matrices to test as well as 5 seeds, we create 15-length lists of the count matrices, meta-information, covariate matrices, and categories. We feed these parameters into a loop which runs each seed-matrix unique combination in parallel.
...
We run 3 covariate matrices, crossed with 5 folds for a total of 15 runs of COMPASS-covariate.
...
We also run original COMPASS with the 5 CD4 training sets. We create a new set of simulation parameters for:

- NS (folds taken from previous): CD4-fold-1, CD4-fold-2, etc.
- NU (folds taken from previous): CD4-fold-1, CD4-fold-2, etc
- meta (folds taken from previous): CD4-fold-1, CD4-fold-2, etc
- seeds
- output directory (orig/CD4)
...
The key objects created in the above sections and at the beginning of this section include
...
The original COMPASS results appear to match the COMPASS heatmap generated from the github object but is missing a few gates at 40000 iterations. The COMPASS-covariate object seems to add gates to be considered. 
...
covarTest <- COVtoCOMPASSObj(CD4CovarCOMPASS_run, 
                ...
                CD4surfaceDF.full.pca %>% as.matrix())
...
In the data, the resulting PFS scores seem more well-separated using COMPASS-covariate in comparison to original COMPASS.

```{r pfs-compare, options}
full.Covar.PFS <- getPFSMetaDF(covarTest) %>% select(Cohort, PFSCovar)
full.Orig.PFS <- getPFSMetaDF(CD4OrigCOMPASS_run$COMPASSobj) %>% select(Cohort, PFSOrig)

wilcox.test(full.Covar.PFS %>% filter(Cohort=="Hospitalized") %>% .$PFSCovar,
            full.Covar.PFS %>% filter(Cohort=="Non-hospitalized") %>% .$PFSCovar)

wilcox.test(full.Orig.PFS %>% filter(Cohort=="Hospitalized") %>% .$PFSOrig,
            full.Orig.PFS %>% filter(Cohort=="Non-hospitalized") %>% .$PFSOrig)
```

### Wilcoxon: PI

However, PFS doesn't seem to do as well as PI in differentiating hospitalized and non-hospitalized patients.

```{r pi-p, options}
numCyt <- 7
PI.score.full<- CD4OrigCOMPASS_run$COMPASSobj$data$meta %>%
                      cbind(PIScore=apply(Percent.Matrix(CD4OrigCOMPASS_run$COMPASSobj$data$n_s, numCyt), 1,
                                          function(row) PI.Score(row, numCyt, 5)))

PI.score.full %>%
  ggplot(aes(x=Cohort, y=PIScore)) + geom_boxplot() + geom_point()


wilcox.test(PI.score.full %>% filter(Cohort=="Non-hospitalized") %>% .$PIScore,
            PI.score.full %>% filter(Cohort=="Hospitalized") %>% .$PIScore)

PI.uniMod <- glm(Cohort_Bin ~ PIScore,
    data=PI.score.full %>% mutate(Cohort_Bin=(Cohort=="Hospitalized")*1),
    family="binomial")

summary(PI.uniMod) #p=0.002
```

### All-var logged, unlogged, and PCA

We compare 5-fold ROC among runs using all-var unlogged covariates, PCA covariates, and all-var logged covariates. Like for RV144, we test different methods of generating predictions for hospitalization. 
...
We run COMPASS-covar on the test data, across the different transformations of the covariate matrix and different folds.
...
```{r test-compass-covariate-run, options}
...
We do the same with original COMPASS.
...
We then read the test COMPASS/COMPASS-covar objects in.
...
```{r compass-cov-funcs, options}
COVID.train.orig <- lapply(1:5, function(i) CD4Orig[[i]]$COMPASSobj)
COVID.test.orig <- lapply(1:5, function(i) CD4Orig.test[[i]]$COMPASSobj)
COVID.train.COMPASScovarObjs <- lapply(1:5, function(i) COVtoCOMPASSObj(CD4Objs_all_vars_log[[i]], COVID.train.orig[[i]], CD4surfaceDFs.log[[i]]))
COVID.test.COMPASScovarObjs <- lapply(1:5, function(i) COVtoCOMPASSObj(CD4Objs_all_vars_log.test[[i]], COVID.test.orig[[i]], CD4surfaceDFs.test.log[[i]]))


COVID.wTestRun.Results <- lapply(1:5,
                                 function(i) rocs_Calc(COVID.train.COMPASScovarObjs[[i]], COVID.test.COMPASScovarObjs[[i]], COVID.train.orig[[i]], COVID.test.orig[[i]], 
          c("PFSCovar"), c("PFSOrig"), "Cohort", "Hospitalized", model="glm"))

COVID.wTestRun.DF <- do.call(rbind, lapply(COVID.wTestRun.Results, function(result) COMPASSTestRun.Results(result, model="glm"))) 
...
We use a regression to predict the status (hospitalized vs. non-hospitalized) based on the estimated PFS of a patient. 

To test each fold's ability to estimate PFS/FS and then status for each test patient:
1. We pull beta samples from the train beta posterior for each of the test samples. 
2. We predict gamma by multiplying measured antibodies by the beta sample coefficients (and adding beta sample intercept), then taking inverse logit.
2. We use gamma to calculate estimated PFS and FS for test.


We aggregate the results of sampling beta from training as follows: taking the mean/median/mode of beta and predicting gamma from that.

We sample entire beta matrices and calculate the mean beta matrix. We then multiply by the covariate test matrix and take inverse-logit to get the mean gamma matrix for the test patients. The mean gamma matrix is used to find the estimated PFS for each patient.
...
```{r mg-est, options}
# get mean betas
COVID.betas <- lapply(1:5, function(i) apply(COVID.train.COMPASScovarObjs[[i]]$fit$beta, c(1,2), mean))
PredMG.COVID.test.COMPASScovarObjs <- lapply(1:5, function(i) getPredGamma(COVID.test.COMPASScovarObjs[[i]], COVID.betas[[i]]))

COVID.wPredMG.Results <- lapply(1:5,
                                 function(i) rocs_Calc(COVID.train.COMPASScovarObjs[[i]], PredMG.COVID.test.COMPASScovarObjs[[i]], 
                                                       COVID.train.orig[[i]], COVID.test.orig[[i]], 
         c("PFSCovar" ), c("PFSOrig" ), "Cohort", "Hospitalized", model="glm"))


COVID.wPredMG.DF <- do.call(rbind, lapply(COVID.wPredMG.Results, function(result) COMPASSTestRun.Results(result, model="glm"))) 
...
#### Averaging over Predicted Indicators

We look at beta for each iteration of the MCMC and use it to calculate the mean gamma matrix, then predicted hospitalization, for each patient.

```{r label, options}
COVID.wPostPredInf.Results <- lapply(1:5,
                                 function(i)rocs_Calc_Avg(COVID.train.COMPASScovarObjs[[i]],
                                                          COVID.test.COMPASScovarObjs[[i]],
                                                          COVID.train.orig[[i]],
                                                          COVID.test.orig[[i]],
                                                          c("PFSCovar" ), c("PFSOrig" ),
                                                          "Cohort", "Hospitalized",
                                                          nSamps=100, model="glm"))


COVID.wPostPredInf.DF <- do.call(rbind, lapply(COVID.wPostPredInf.Results, 
                                               function(result) COMPASSPostPred.Results(result,
                                                                                    model="glm"))) 

COVID.wPostPredInf.DF %>%
  kbl() %>%
  kable_styling()
...
We can either use PI alone in a GLM (done below) or aggregate these PI scores to meta information and antibody covariates and run a LASSO.

We then get the ROC associated with the PI+covariate model for the test and train data.

```{r pi-roc, options}
numCyt <- 7
COVID.wPI.Results <- lapply(1:5, function(i) rocs_Calc_PI(COVID.train.orig[[i]], COVID.test.orig[[i]], 
             c("PI" ), "Cohort", "Hospitalized",
             q=5, model="glm"))

COVID.wPI.DF <- do.call(rbind, lapply(COVID.wPI.Results, function(result) PIRun.Results(result, model="glm"))) 
...
```{r viz-auc, options}
COVID.wTestRun.DF %>%
  cbind(COVID.wPredMG.DF %>% select(TestCovarROC) %>% rename(TestPredMGROC=TestCovarROC)) %>%
  cbind(COVID.wPostPredInf.DF %>% select(TestCovarROC) %>% rename(TestPostPredInfROC=TestCovarROC)) %>%
  cbind(COVID.wPI.DF %>% select(TestOrigROC) %>% rename(TestPIROC=TestOrigROC)) %>%
  ...
  labs(x="ROC Type", title = "ROC over Five Folds of Hospitalization Regression")
```
