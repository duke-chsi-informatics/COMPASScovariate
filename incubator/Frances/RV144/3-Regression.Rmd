...
outcome = scale(as.vector(outcomeDF[sel_outcome,i])) # can replace this with PFS, FS, mean gamma as needed
...
qvalsDF %>%
    mutate(outcomeName = ifelse(str_detect(outcomeName, "&"), map_chr(str_extract_all(outcomeName, "\\!|&\\!|&")
                          , ~ str_c(.x,collapse = "")) %>%
             str_replace_all("\\!|(&\\!)","-") %>% str_replace_all("&","\\+"), outcomeName)) %>%
  # missing + for first cytokine if it is positive
    mutate(outcomeName = ifelse(nchar(outcomeName)==5, paste0("+", outcomeName), outcomeName))
}
...
covarQvals %>%
  ...
  mutate(outcomeName=ifelse(outcomeName=="PFSRV144covar", "PFSRV144", outcomeName)) %>%
  ggplot(aes(x=outcomeName, y=outcome, fill=type)) +
  geom_bar(stat="identity", position = "dodge") +
  ylab("Q-value") +
  xlab("Outcome") +
  ggtitle("Q-values for Covar vs. Original Outcomes with Q-value < 0.1")
```
...
grid.arrange(origVolc, covarVolc)
