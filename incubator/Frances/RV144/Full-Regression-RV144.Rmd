...
# Program: /CHSI/PackageDev/COMPASScovariate/incubator/Frances/RV144/Full-Regression-RV144

#

# Description: Regression analysis on RV144 mirroring what was done in the COMPASS paper. Adding COMPASS-covar.
#--- --- --- --- --- --- --- --- --- --- --- --- ---
...
# Correlation of Luminex with Antibody covariates

```{r lum-corr, options}
extendMeta <- function(fit, dat, orig, PFSname, FSname) {
...
#meta should be already ordered by PTID like mean gamma; get rid of all-zero columns
meta <- fit$data$meta %>% as.data.frame() 
...
Lum = meta[sel_outcome,] %>% .$Luminex


return(data.frame(Lum=Lum,
                  flrstatus=flrstatus,
                  IgAprim=IgAprim,
                  V2prim=V2prim,
                  NAbprim=NAbprim,
                  ...
                  risk.high=risk.high,
                  stratuminds=stratuminds))
}

covarAntibodies <- extendMeta(covarfit, dat, orig,  "PFSRV144covar", "FSRV144covar") 

#########################################################
# Correlation between antibodies and Luminex
#########################################################
covarAntibodies %>%
  select(IgAprim, V2prim, NAbprim, Avidprim, ADCCprim, Lum) %>% cor() %>%
  corrplot()
