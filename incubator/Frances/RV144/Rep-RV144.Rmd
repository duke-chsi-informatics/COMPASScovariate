...
#source("~/CHSI/PackageDev/COMPASScovariate/R/COMPASS-discrete.R")
...
COMPASScovarV2Lum<- readRDS("~/CHSI/PackageDev/COMPASScovariate/incubator/Frances/Data/rv144COMPASScovar200000_seed100_Lum.rds")$COMPASScovarobj
...
diffquants %>%
  ...
  geom_point() + facet_wrap(.~Gate, scales="free")

changingPats %>% 
  select(-c(OGlessThreshold, CovarlessThreshold, Flag)) %>%
# join with Luminex and standardized differences of flow gates
  left_join(antibodyDF_transf)  %>%
  left_join(flow_freqDiffs(flow_agg) %>%
  ungroup() %>%
  pivot_longer(cols = -c(PTID), names_to = "Gate", values_to = "values") %>%
    simpGates() %>%
  drop_na()) %>%
# scale differences over all gates
  # simpGates() %>% mutate(`Standardized Gate Diffs`=scale(values))) %>%
  #select(-values) %>%
  arrange(desc(abs(Covar-Original))) %>%
# find gate diff means/sds
  group_by(Gate) %>%
  mutate(GateMean=mean(values),
         GateSD = sd(values))  %>%
# find quantile of where value lies on gate diff
  mutate(valsQuant = percent_rank(values)) %>%
  datatable(rownames = FALSE, options = list(
  pageLength = 5, autoWidth = TRUE))
...
flow_freqDiffs(flow_agg) %>%
  ...
  facet_wrap(.~Gate,scales="free_x", ncol = 3) +
