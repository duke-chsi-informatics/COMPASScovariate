...
diffquants %>%
  ...
  ggplot(aes(x=quantDiff, y=Luminex, color=mean_gam))+
  geom_point(alpha=0.4) + facet_wrap(.~Gate) +
  labs(x="Quantile of Difference in Subset %", y= "Standardized Luminex",
       color="Original Mean Gamma") +
  ggtitle("Original Mean Gamma", subtitle = "by Subset % Difference (quanticized) and Luminex")

ggsave("./Regression/Plots/quantDiffsvLumMG.png", width = 10, height = 9)
...
diffquants %>%
  ...
  group_by(Gate) %>%
  mutate(Range=max(values)-min(values)) %>% ungroup() %>%
  mutate(lab=paste0(Gate, ", ", round(V2,1), ": ", round(Range,5))) %>%
...
changingFlow <-flow_freqDiffs(flow_agg) %>%
  ...
  dplyr::rename(`Lower Covariate Mean Gamma` = CovarlessThreshold) %>%
  filter(Flag!="None") %>%
