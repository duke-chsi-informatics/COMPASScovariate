...
# Program: Rep-RV144.Rmd

#

# Description: Exploration (including convergence measures, heatmaps of beta coefficients, boxplots of scores, etc.) of RV144 COMPASS/COMPASS-covar results.
#--- --- --- --- --- --- --- --- --- --- --- --- ---
...
We log-scale Luminex because it ranges from 0-1. The below histogram  shows the untransformed covariates and the violin plot shows the covariates after scaling the logged Luminex (if Luminex = 0, then we take log(Luminex + 1)).
...
antibodyDF_transf <- antibodyDF %>%
  mutate(Luminex = scale(log(Luminex + 1)))
...
n_s <- flow_agg %>% filter(Stimulation=="Stim") %>%
  ...
  #--------------------------------------------------------
  # Full run of COMPASS/COMPASS-covar
  #--------------------------------------------------------
  
  # runCOMPASS(n_s, n_u, "PTID", meta, X, categories=getCats(n_s), iter=1000, seed=47,
  #            outdirorig="/work/fwh6/RV144/Full", outdircovar="/work/fwh6/RV144/Full",
  #            dataName="rv144Full")
...
COMPASSorig<- readRDS("/work/fwh6/RV144/Full/rv144Full1000_seed47.rds")$COMPASSobj$fit
...
COMPASScovarV2Lum<- readRDS("/work/fwh6/RV144/Full/rv144Fullcovar1000_seed47.rds")$COMPASScovarobj
...
We plot the Geweke's, Raftery and Lewis, and Heidelberg and Welch statistic for the two gates' patients' gamma values.
...
# 
# #hd <- heidel.diag(COMPASScovarV2Lum$gamma[patsForGeweke$pat_idx,
# #                                          patsForGeweke$names_idx,])
# 
# patsForGeweke %>%
...
We also can use the Raftery and Lewis statistic
...
```{r hm, options}
parentDirectory <- "~/CHSI/PackageDev/COMPASScovariate/incubator/Frances/RV144/"
COMPASSobj <- readRDS("/work/fwh6/RV144/Full/rv144Full1000_seed47.rds")$COMPASSobj


# make sample ID rownames in meta
#rownames(COMPASSobj$data$meta) <- COMPASSobj$data$meta$PTID

# #--------------------------------------------------------
# # Original Heatmap
# #--------------------------------------------------------
# 
# png(file=file.path(parentDirectory,"2-Output/RV144_orig.png"),
#     width=500, height = 550)
# 
# origplot <- plot.COMPASSResult.ComplexHeatmap(COMPASSobj,
#                                   row_annotation = "flrstatus",
#                                   row_annotation_colors = list(
#                                     "Uninfected"=my_palette[11],
#                                     "Infected"=my_palette[12],
#                                     "Placebo" = my_palette[13]))
# 
# draw(origplot)
# dev.off()
# 
...
covarTest <- list(fit=COMPASScovarV2Lum,
                  data=COMPASSobj$data)
# make sample ID rownames in meta
rownames(covarTest$data$meta) <- covarTest$data$meta$PTID
...
colnames(covarTest$fit$beta) <- c("Intercept", colnames(X))
# 
# my_palette <- pal_igv("default", alpha=0.7)(15)
# 
# 
# png(file=file.path(parentDirectory,"2-Output/RV144_covar.png"),
#     width=500, height = 550)
# covarplot <- plot.COMPASSCovarResult.ComplexHeatmap(covarTest,
#                                        covar_to_plot = "Luminex",
#                                        row_annotation = "flrstatus",
#                                   row_annotation_colors = list(
#                                     "Uninfected"=my_palette[11],
#                                     "Infected"=my_palette[12],
...
We plot the PI calculated for the placebo vs. vaccinated patients and calculate the wilcoxon p-value for difference in PI between placebo and vaccinated. It looks like the PI matches the Supplementary Figure 2 in the original COMPASS paper.

```{r PI-plot, options}
numCyt=6
q=1

PI.percMat <- Percent.Matrix(COMPASSobj$data$n_s, 6)
fullPI <- apply(Percent.Matrix(COMPASSobj$data$n_s, 6), 
                1, function(row) PI.Score(row, numCyt, q))

PIDF <- data.frame(PTID=rownames(COMPASSobj$data$n_s), PI=fullPI) %>%
  left_join(COMPASSobj$data$meta %>% select(infect, vaccine, PTID))
  
PIDF %>%
  ggplot(aes(x=vaccine, y=PI)) + geom_boxplot() + geom_point()

PIDF %>%
  ggplot(aes(x=infect, y=PI)) + geom_boxplot() + geom_point()

# p-value=0.45 
wilcox.test(PIDF %>% filter(infect=="NON-INFECTED") %>% .$PI, PIDF %>% filter(infect=="INFECTED") %>% .$PI)
...
PFSOrig <- PolyfunctionalityScore(COMPASSobj) %>% as.matrix()
COMPASScovarV2Lumtest <-COVtoCOMPASSObj(list(COMPASScovarobj=COMPASScovarV2Lum), COMPASSobj, X)
PFSCovar <- PolyfunctionalityScore.COMPASSResult(COMPASScovarV2Lumtest) %>% as.data.frame()
...
wilcox.test(RV144PFS_DF %>% filter(infect=="INFECTED") %>% .$PFSRV144,
            RV144PFS_DF %>% filter(infect=="NON-INFECTED")%>% .$PFSRV144)

wilcox.test(RV144PFS_DF %>% filter(infect=="INFECTED") %>% .$PFSRV144covar,
            RV144PFS_DF %>% filter(infect=="NON-INFECTED") %>% .$PFSRV144covar)
```
...
RV144PFS_DF %>%
  select(PTID, PFSRV144, PFSRV144covar, vaccine) %>%
  pivot_longer(cols = -c(PTID, vaccine), names_to = "Type", values_to = "Value") %>%
  ggplot(aes(x=Type, y = Value, color = vaccine)) + geom_boxplot(outlier.shape = NA) + geom_point(position = position_jitterdodge(0.05, seed = 47)) +
  ylab("Polyfunctional Score")
...
```{r paired-box, options}

RV144PFS_DF <- data.frame(PFSRV144=PolyfunctionalityScore(COMPASSobj),
                             PFSRV144covar=PolyfunctionalityScore.COMPASSResult(covarTest))%>%
  rownames_to_column(var="PTID") %>%
...
# joining with demographic information
  inner_join(meta)  %>%
# remove placebos
  filter(!is.na(flrstatus)) %>%
# relabel
  mutate(flrstatus=ifelse(flrstatus==0, "Uninfected", "Infected"))
...
RV144PFS_DF %>% 
  ...
  geom_line(aes(group=PTID, color=flrstatus),alpha=0.5) +facet_wrap(.~flrstatus) +
  labs(color="Infection Status", x="COMPASS Type")

ggsave(filename=file.path(parentDirectory,"2-Output/RV144_PFS.png"),
       height = 5, width = 6)
```
...
FSRV144_DF %>%
  select(PTID,  FSRV144, FSRV144covar, vaccine) %>%
  pivot_longer(cols = -c(PTID, vaccine), names_to = "Type", values_to = "Value") %>%
  ggplot(aes(x=Type, y = Value, color = vaccine)) + geom_boxplot(outlier.shape = NA) + geom_point(position = position_jitterdodge(0.1, seed = 47)) +
  ylab("Functional Score")

wilcox.test(FSRV144_DF %>% filter(infect=="INFECTED") %>% .$FSRV144,
            FSRV144_DF %>% filter(infect=="NON-INFECTED") %>% .$FSRV144)

wilcox.test(FSRV144_DF %>% filter(infect=="INFECTED") %>% .$FSRV144covar,
            FSRV144_DF %>% filter(infect=="NON-INFECTED") %>% .$FSRV144covar)
```

### Comparing to the original object

The scores here look correct. Is there variation due to the 

```{r orig-PFS, options}
PFSOG<- PolyfunctionalityScore(rv144) %>% as.matrix()
FSOG <- FunctionalityScore.COMPASSResult(rv144) %>% as.matrix()

OGscoreDF <- data.frame(PTID=rownames(PFSOG), 
                        PFS=PFSOG[,1],
                        FS = FSOG[,1]) %>%
  left_join(rv144$data$meta %>% select(flrstatus, PTID, infect, vaccine))
  
OGscoreDF %>%
  ggplot(aes(x=vaccine, y=PFS)) + geom_boxplot() + geom_jitter()

# p=0.0112
wilcox.test(OGscoreDF %>% filter(infect=="INFECTED") %>% .$PFS,
            OGscoreDF %>% filter(infect=="NON-INFECTED")%>% .$PFS)

# p=0.0298
wilcox.test(OGscoreDF %>% filter(infect=="INFECTED") %>% .$FS,
            OGscoreDF %>% filter(infect=="NON-INFECTED")%>% .$FS)
```
