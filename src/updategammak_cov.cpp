...
// [[Rcpp::export]]
SEXP updategammak_cov(SEXP n_s,
                       SEXP n_u,
                       SEXP gammat,
                       SEXP I,
                       SEXP K,
  ...
  Rcpp::IntegerMatrix xWK1(WK1);
  Rcpp::IntegerMatrix xWK0(WK0);
    ...
    for (int ss = 0; ss < xSS; ss++) {
      if (xindiK1 > 0) {
        ...
        for (int j = 0; j < K1; j++) {
          ...
          log_likeli_deno +=
            gamma_old[j] * xWK1(i, j) +
            (1 - gamma_old[j]) * xWK0(i, j);
        }
        if (flag > 1) {  //*
          Iistar = xIstar - 1;
        ...
        for (int k = 0; k < K1; k++) {
          log_likeli_nume +=
            gamma_tt(i, k) *xWK1(i, k) +
            (1 - gamma_tt(i, k)) * xWK0(i, k) ;
          if (gamma_tt(i, k) == 0) {
            lplace0_new += 1;
          }
          if (gamma_old[k] == 0) {
